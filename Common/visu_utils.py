...
def plot_pcd_three_views_color(filename, pcds, titles, suptitle='', sizes=None, cmap=['Reds'], zdir='y',
                         xlim=(-0.4, 0.4), ylim=(-0.4, 0.4), zlim=(-0.4, 0.4),color=None):
    if sizes is None:
        sizes = [0.2 for i in range(len(pcds))]
    fig = plt.figure(figsize=(len(pcds) * 3, 9))
    for i in range(3):
        elev = 30
        azim = -45 + 90 * i
        for j, (pcd, size,c_m,c) in enumerate(zip(pcds, sizes,cmap,color)):
            #color = pcd[:, 0]
            ax = fig.add_subplot(3, len(pcds), i * len(pcds) + j + 1, projection='3d')
            ax.view_init(elev, azim)
            ax.scatter(pcd[:, 0], pcd[:, 1], pcd[:, 2], zdir=zdir, c=c, s=size, cmap=c_m, vmin=-1, vmax=0.5)
            #print(j,titles[j])
            if i == 0:
                ax.set_title(titles[j])
            ax.set_axis_off()
